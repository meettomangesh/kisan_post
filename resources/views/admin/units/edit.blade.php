        ...
        <form method="POST" action="{{ route("admin.units.update", [$unit->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-body">
                    ...
                    <div class="col-md-4">
                        <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="cat_id" id="cat_id" required>
                            @foreach($categories as $id => $category)
                                <option value="{{ $id }}" {{ ($unit->category ? $unit->category->id : old('cat_id')) == $id ? 'selected' : '' }}>{{ $category }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" rows="2" name="description" id="description">{{ old('description', $unit->description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <div class="radio-list">
                            <label class="radio-inline"><input type="radio" name="status" value="{{ old('status', '1') }}" {{ $unit->status == '1' ? 'checked' : '' }} required> {!! trans('cruds.unit.fields.active') !!}</label>
                            <label class="radio-inline"><input type="radio" name="status" value="{{ old('status', '0') }}" {{ $unit->status == '0' ? 'checked' : '' }} required> {!! trans('cruds.unit.fields.inactive') !!}</label>
                        </div>
                    </div>
                </div>
            </div>
